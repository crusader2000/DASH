...
def header():
    ...
    print(
        '    [ Only if Ngrok is not configured on your Device. ]{}\n'.format(
            colors.red,
            colors.white,
            colors.white))
    print(
...
if __name__ == '__main__':
    ...
    if os == 'Linux' or 'Darwin':
        ...
        ngrok = subprocess.Popen(['wget {}'.format(
            ngrok_download[os])], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
        for output in ngrok.stdout:
            if "OK" in output.decode('utf-8'):
                print(
                    "{}[1/3]{} Download Started".format(colors.green, colors.white))
            if "%" in output.decode(
                    'utf-8') and len(output.decode('utf-8').split(" ")[-3]) > 0:
                print("   Downloaded..... : ", colors.green,
                      output.decode('utf-8').split(" ")[-3], colors.white)

        print(
            "\n{}[2/3]{} Unziping & moving ngrok to bin".format(colors.green, colors.white))
        unzip = subprocess.Popen(['unzip {}'.format(ngrok_download[os].split(
            '/')[-1])], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
        ...
        token = input(
            "\nEnter Ngrok Authentication token : {}".format(
                colors.green))

        ng_auth = subprocess.Popen(['ngrok authtoken {}'.format(
            token)], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
